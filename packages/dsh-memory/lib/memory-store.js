...
export class MemoryStore {
  ...
  clear() {
    try {
      ...
      console.error("[dsh-memory] memory clear failed:", error);
    }
  }
  ...
  #migrate(dir) {
    ...
    if (existsSync(legacy)) {
      try {
        ...
        rmSync(legacy);
        console.log("[dsh-memory] migrated legacy memory.json → memory/ (flat)");
      } catch (error) {
        console.error("[dsh-memory] legacy memory migration failed:", error);
      }
    }
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (count > 0) console.log(`[dsh-memory] consolidated memory/${entry.name}/ → memory/ (${count} notes)`);
      }
    } catch {

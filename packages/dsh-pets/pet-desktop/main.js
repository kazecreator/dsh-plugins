const { app, BrowserWindow, Menu, ipcMain, screen, shell, nativeImage } = require("electron");
...
function createPetWindow() {
  ...
  // Transparent frameless windows have no native title-bar controls. Keep the
  // browser context menu out of the way and provide the one window action the
  // pet needs: closing it. The settings page can launch it again later.
  win.webContents.on("context-menu", (event) => {
    event.preventDefault();
    const menu = Menu.buildFromTemplate([
      { label: "关闭宠物", click: closePet },
    ]);
